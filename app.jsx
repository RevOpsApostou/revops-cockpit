...
function buildFarolGroups_(MM, f, range, useYtd, sparkByKey, scenLabel) {
  ...
  return [
    ...
    // Margem por safra: qualidade de monetização por IDADE DE COORTE (não é retenção) — janela MTD.
    // Só entra card com valor: bucket sem safra no período sai da tela em vez de virar um "—" mudo
    // (e sem ggrSafra nenhum a seção inteira desaparece).
    { title: 'Margem por safra', cards: [
...
function buildFarolExportGroups_(MM, f, monthlyClose, channels, chFilter, range, useYtd) {
  ...
  ['Aquisição', 'Depósito M0', 'Volume & GGR', 'Retenção', 'Margem por safra'].forEach(t => { if (!G[t]) G[t] = {}; });
  ...
  return [
    ...
    // Margem por safra: sai igual à tela (sem splits/coortes absolutas).
    { title: 'Margem por safra', cards: (groups.find(g => g.title === 'Margem por safra') || {}).cards || [] },
...
function buildFarolMetrics_(M, comp, channels, ggrChannels, bp, filter, ggrSafra) {
  ...
  const safraMargem = {};
  [['m0', 'M0'], ['m1', 'M1'], ['m2', 'M2'], ['m3plus', 'M3+']].forEach(([bk, lbl]) => {
    ...
    safraMargem['ggrDep_' + bk] = mk(`GGR/Dep ${lbl}`, 'pct', on ? div(ggr, dep) : null, null, on ? div(ggrL, depL) : null);
    safraMargem['hold_' + bk]   = mk(`Hold ${lbl}`,    'pct', on ? div(ggr, turn) : null, null, on ? div(ggrL, turnL) : null);
  });
